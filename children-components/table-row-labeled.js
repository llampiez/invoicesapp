...
export class TableRowLabeled extends StyledComponent {
  ...
  render() {
    ...
    const labelFontSize =
      this.getAttribute('label-font-size') ?? '1rem';
    const labelFontWeight =
      this.getAttribute('label-font-weight') ?? 'normal';
    const labelColor = this.getAttribute('label-color') ?? 'black';
    const labelMarginBottom =
      this.getAttribute('label-margin-bottom') ?? '0';
...
customElements.define('tr-labeled', TableRowLabeled);
